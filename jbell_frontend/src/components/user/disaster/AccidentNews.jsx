  ...
  return (
    <div className="grid grid-cols-12 gap-5 lg:gap-6">
        ...
        <div className="bg-white rounded-xl shadow-1 border border-graygray-10 flex flex-col lg:flex-row overflow-hidden h-auto lg:h-[480px]">
          ...
          <div className="w-full lg:w-[280px] border-b lg:border-b-0 lg:border-r border-graygray-10 p-5 flex flex-col gap-4 z-20 bg-white">
            ...
            <div className="relative">
              <label className="text-[11px] font-bold text-graygray-40 ml-1 mb-1 block">
                재난사고구분
              </label>
              <button
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                className={`w-full bg-white border ${
                  isDropdownOpen ? "border-blue-600 ring-2 ring-blue-50" : "border-graygray-10"
                } px-4 py-2.5 rounded-xl flex justify-between items-center text-body-m font-bold text-graygray-80 transition-all shadow-sm`}
              >
                {selectedCategory}
              ...
              {isDropdownOpen && (
                <div className="absolute top-[105%] left-0 w-full bg-white border border-graygray-10 shadow-xl rounded-xl py-2 z-[100]">
                  {categories.map((cat) => (
                    <button
                      key={cat}
                      onClick={() => {
                      ...
                      className="w-full text-left px-4 py-2 text-body-m font-bold text-graygray-60 hover:bg-secondary-5 hover:text-blue-600 transition-colors"
                    >
                      {cat}
                    </button>
                  ))}
              ...
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="bg-white border border-graygray-10 rounded-xl overflow-hidden shadow-sm hover:border-blue-300 transition-all group cursor-pointer">
                  <div className="bg-secondary-5/50 px-3 py-1.5 border-b border-graygray-5">
                    <span className="text-[10px] font-bold text-graygray-40">2026.01.02 16:10</span>
                  </div>
                  <div className="p-3 flex flex-col items-center text-center">
                    <h5 className="text-body-s-bold text-graygray-90 mb-2 leading-tight">위치 : ㅇㅇㅇ 아파트 앞</h5>
                    <div className="inline-flex items-center gap-1.5 bg-white text-red-600 px-3 py-1 rounded-full text-[10px] font-black border border-red-500">
                      화재 진압중
                    </div>
                  </div>
                </div>
          ...
          <div className="flex-1 min-h-[350px] lg:min-h-full bg-secondary-5 relative overflow-hidden flex items-center justify-center">
            <span className="text-title-s font-black text-graygray-20 tracking-[0.2em] uppercase">Map Area</span>
          </div>
        </div>
      ...
      <div className="col-span-12 lg:col-span-4 flex flex-col gap-5 lg:gap-6 items-center lg:items-start">
        <div className="w-full max-w-[370px] h-[200px] bg-gradient-to-br from-[#62A1E9] to-[#4A90E2] rounded-xl p-6 shadow-1 border border-white/30">
          <WeatherBox />
        </div>
