...
const AdminSubCodeEdit = () => {
  ...
  const [formData, setFormData] = useState({
    parentGroupInfo: '', subCodeId: '', subCodeName: '', desc: '', order: 1, regDate: '', modDate: ''
  });
  ...
  useEffect(() => {
    ...
    if (detailData) {
      ...
      const initialForm = {
        parentGroupInfo: parentGroup ? `${parentGroup.groupCode} (${parentGroup.groupName})` : `${detailData.groupCode} (${detailData.groupName})`,
  ...
  const checkDuplicateName = useMemo(() => {
    ...
    if (!currentInputName || (originalData && currentInputName === originalData.subCodeName.trim())) return false;
    return AdminCommonCodeData.some(item => 
      item.groupCode === currentGroupCode && item.subCode !== '-' && 
      String(item.id) !== String(id) && item.subName.trim() === currentInputName
    );
  }, [formData.subCodeName, originalData, id, currentGroupCode]);

  const isNameEmpty = isSubmitted && !formData.subCodeName.trim();
  ...
  const handleSave = () => {
    ...
    if (!formData.subCodeName.trim()) {
      alert("필수 입력 사항을 모두 작성해주세요."); // alert 추가
      return;
    }
    if (checkDuplicateName) {
      alert("동일한 그룹 내에 이미 존재하는 상세 코드명입니다."); // alert 추가
      return;
    }
    setIsModalOpen(true);
  };
  ...
  const handleConfirmSave = () => {
    ...
    if (targetIndex !== -1) {
      ...
      AdminCommonCodeData[targetIndex] = { ...AdminCommonCodeData[targetIndex], subName: finalName, desc: formData.desc.trim(), order: formData.order, visible: isRegistered, editDate: formattedDate };
      setBreadcrumbTitle(finalName);
    }
    setToastMessage("상세코드가 성공적으로 수정되었습니다.");
    setShowToast(true);
    setTimeout(() => { setShowToast(false); navigate(`/admin/system/subCodeDetail/${id}`, { replace: true }); }, 1500);
  };

  const handleCancel = () => {
    if (isDirty) { setIsCancelModalOpen(true); } 
    else { navigate(`/admin/system/subCodeDetail/${id}`, { replace: true }); }
  };
  ...
  const confirmCancel = () => {
    ...
    setShowToast(true);
    setTimeout(() => { setShowToast(false); navigate(`/admin/system/subCodeDetail/${id}`, { replace: true }); }, 1000); 
  };
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-10 w-full max-w-[500px]">
              ...
              <input 
                ...
                className={`w-full border rounded-lg px-5 py-4 outline-none transition-all font-medium ${isNameEmpty || checkDuplicateName ? 'border-[#E15141] ring-1 ring-red-50' : 'border-gray-300 focus:border-[#2563EB]'}`} 
              />
              <div className="flex justify-between items-start mt-2">
                <div className="flex-1 min-h-[20px]">
                  {isNameEmpty ? (
                    ...
                    <div className="text-[#E15141] text-sm flex items-center gap-2 font-medium"><ErrorIcon /> 이미 존재하는 상세 코드명입니다.</div>
                  ) : (
                    <p className="text-[13px] text-gray-400 font-medium">* 최대 20자까지 입력 가능</p>
                  )}
                </div>
                <span className="text-[12px] text-gray-400 font-medium ml-4 shrink-0">{formData.subCodeName.length} / 20</span>
              </div>
            </div>
