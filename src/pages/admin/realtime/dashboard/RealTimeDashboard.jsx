...
export const RealTimeDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const getListData = (res) =>
          res.data?.list || res.data?.data || res.data || [];

        setAllMessages(getListData(msgRes));
        setAllWarnings(getListData(warningRes));

        const coldData = getListData(coldRes).map((d) => ({
          ...d,
          unifiedDate: d.tmFc,
        }));
        const fireData = getListData(fireRes).map((d) => ({
          ...d,
          unifiedDate: d.fireStartTime,
        }));
        const eqData = getListData(eqRes).map((d) => ({
          ...d,
          unifiedDate: d.TM_EQK || d.tmEqk || d.tmFc,
        }));
  ...
  const disasterStats = useMemo(
    () => getStatInfo(allDisasters, "unifiedDate"),
    [allDisasters, timeRange],
  );
  const messageStats = useMemo(
    () => getStatInfo(allMessages, "crtDt"),
    [allMessages, timeRange],
  );
  const warningStats = useMemo(
    () => getStatInfo(allWarnings, "PRSNTN_TM"),
    ...
    <div className="relative w-full h-[950px] overflow-hidden mx-auto border border-gray-100 font-sans">
      ...
      <section className="absolute top-[108px] left-[50px] right-[50px] flex justify-between gap-4">
        {statCards.map((card) => (
          ...
          >
            <div
              className={`${card.bg} p-4 rounded-xl flex items-center justify-center shrink-0`}
            >
              {card.icon}
            </div>
            <div className="flex flex-col gap-1 overflow-hidden">
              <h3 className="font-semibold text-gray-500 text-[15px] whitespace-nowrap">
                {card.title}
              </h3>
              <p className="font-bold text-[#1d1d1d] text-[24px]">
                {card.count}
              </p>
            </div>
