...
export const RealTimeDashboard = () => {
  ...
  const [allWarnings, setAllWarnings] = useState([]); // 기상 특보 상태 추가
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 재난 문자 및 기상 특보 동시 호출
        const [msgRes, warningRes] = await Promise.all([
  ...
  const filterByTime = (data, dateField) => {
    return data.filter(item => {
      ...
      // PRSNTN_TM (202601281400) 또는 crtDt (2026/01/28...) 형식 대응
      let itemDate;
      if (typeof dateVal === 'string' && dateVal.includes('/')) {
        itemDate = new Date(dateVal.replace(/\//g, '-'));
      } else if (String(dateVal).length >= 8) {
        // YYYYMMDDHHmm 형식을 Date 객체로 변환
        const s = String(dateVal);
        itemDate = new Date(`${s.substring(0, 4)}-${s.substring(4, 6)}-${s.substring(6, 8)}T${s.substring(8, 10)}:${s.substring(10, 12)}:00`);
      } else {
        return false;
      }
      ...
      if (timeRange === "최근 24시간" || timeRange === "1일") return diffHours <= 24;
      if (timeRange === "3일") return diffHours <= 72;
      if (timeRange === "7일") return diffHours <= 168;
      if (timeRange === "30일") return diffHours <= 720;
      return true;
    });
  };

  // 필터링된 데이터 계산
  const filteredMessages = useMemo(() => filterByTime(allMessages, 'crtDt'), [allMessages, timeRange]);
  const filteredWarnings = useMemo(() => filterByTime(allWarnings, 'PRSNTN_TM'), [allWarnings, timeRange]);
  ...
  const statCards = [
    ...
    { 
      ...
      bg: "bg-blue-50", 
      title: "기상 특보", 
      count: `${filteredWarnings.length}건`, // 기상 특보 갯수 반영
      statType: "down", 
      statValue: "2", 
    ...
    { 
      ...
      bg: "bg-amber-50", 
      title: "재난 문자", 
      count: `${filteredMessages.length}건`, 
      statType: "up", 
      statValue: "4", 
