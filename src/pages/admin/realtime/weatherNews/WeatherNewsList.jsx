...
const WeatherNewsList = () => {
  ...
  const fetchAdminData = useCallback(async () => {
    ...
    try {
      ...
      const mappedData = allRawData.map((item) => {
        ...
        return {
          id: item.PRSNTN_SN,
          level: item.TTL?.includes("경보")
            ? "위험"
            : item.TTL?.includes("주의보")
              ? "주의"
              : "보통",
  ...
  const getAllSelectedItemsList = () => {
    const selectedItems = weatherNews.filter((item) =>
      selectedIds.includes(item.id),
    );
    return selectedItems
      .map((item) => (item.title || "").substring(0, 10) + "...")
      .join(", ");
  };
    ...
    setModalConfig({
      ...
      message: (
        <div className="flex flex-col gap-2 text-left">
          <p>
            선택하신{" "}
            <span className="text-admin-primary font-bold">[{allNames}]</span>{" "}
            항목을
          </p>
          <p>
            일괄{" "}
    ...
    setModalConfig({
      ...
      message: (
        <div className="flex flex-col gap-2 text-left">
          <p>
            선택하신{" "}
            <span className="text-red-600 font-bold">[{allNames}]</span> 항목을
            정말 삭제하시겠습니까?
          </p>
  ...
  const columns = useMemo(
    () => [
      { key: "id", header: "NO", width: "180px", className: "text-center" },
      {
        key: "type",
        header: "특보유형",
        width: "120px",
            ...
            <div className="flex items-center border border-admin-border rounded-md px-4 h-14 bg-white focus-within:border-admin-primary transition-all shrink-0">
              <div className="flex items-center gap-2">
                <div className="group relative flex items-center w-[130px]">
                  ...
                  <Calendar
                    size={16}
                    className="absolute right-0 text-graygray-30 transition-colors group-hover:text-admin-primary pointer-events-none"
                  />
                </div>
