...
const AdminSideBar = () => {
  ...
  const findActiveCategory = () => {
    return menuCategories.find(cat => 
  ...
  return (
    ...
    <aside className="w-64 bg-[var(--admin-sidebar-bg)] text-gray-300 flex flex-col min-h-screen">
      ...
      <div className="p-6 flex items-center gap-3 border-b border-[#002140]">
        ...
        <span className="text-white font-bold text-lg">대한민국정부</span>
      </div>

      <nav className="flex-1 mt-4 overflow-y-auto custom-scrollbar">
        {menuCategories.map((cat) => (
          <section key={cat.title} className="flex flex-col border-b border-[#002140]/30 last:border-0">
            ...
            >
              ...
              {cat.isAvailable && (
                openCategory === cat.title ? <ChevronUp size={18}/> : <ChevronDown size={18}/>
              )}
            </button>
