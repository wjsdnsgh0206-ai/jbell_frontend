...
/** 1️⃣ 경보 수준 판별 */
const getWarningLevel = (ttl = '') => {
  if (ttl.includes('경보')) return 'WARNING';
  if (ttl.includes('주의보')) return 'WATCH';
  return 'WATCH';
};
...
const getCategories = (ttl = '', content = '') => {
  ...
  const categoryRules = [
    { key: '지진', match: ['지진'] },
    { key: '호우홍수', match: ['호우', '홍수'] },
    { key: '태풍', match: ['태풍'] },
    { key: '산사태', match: ['산사태'] },
    { key: '산불', match: ['산불', '건조'] }, // ✅ 건조 → 산불로 분류
    { key: '한파', match: ['한파', '저온'] },
    { key: '폭염', match: ['폭염'] },
    { key: '강풍', match: ['강풍'] },
    { key: '대설', match: ['대설', '눈'] },
...
const normalizeWarning = (item) => {
  const level = getWarningLevel(item.TTL);
  const categories = getCategories(item.TTL, item.RLVT_ZONE);
...
const WeatherWarningBox = ({ disasterType }) => {
  ...
  const filteredWarnings = useMemo(() => {
    ...
    return warnings
      ...
      .filter(item => item.categories.includes(targetCategory));
            ...
            >
              ...
              <div className="flex justify-between items-start mb-2">
                ...
                >
                  ...
                  {item.level === 'WARNING' ? '위험(경보)' : '주의(주의보)'}
                </span>
